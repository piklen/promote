...
# 最终修复和部署脚本 v2.0
# 解决所有已知问题：cryptography版本、nginx用户冲突、package-lock.json缺失等
...
echo -e "${PURPLE}🎯 这是最终版本的修复脚本，解决所有已知问题${NC}"

# 1. 停止现有容器
echo -e "${YELLOW}🛑 步骤 1/10: 停止现有容器...${NC}"
docker-compose -f docker-compose.prod.yml down --remove-orphans 2>/dev/null || true

# 2. 全面清理Docker资源
echo -e "${YELLOW}🧹 步骤 2/10: 全面清理Docker资源...${NC}"
...
# 3. 拉取最新代码
echo -e "${YELLOW}📥 步骤 3/10: 拉取最新代码...${NC}"
...
# 4. 确保依赖文件完整
echo -e "${YELLOW}📋 步骤 4/10: 检查依赖文件...${NC}"
...
# 5. 检查后端依赖
echo -e "${YELLOW}📋 检查后端依赖文件...${NC}"
if grep -q "cryptography==41.0.8" backend/requirements.txt 2>/dev/null; then
    echo -e "${YELLOW}⚠️ 发现老版本的cryptography，但应该已经修复...${NC}"
fi

# 6. 创建必要目录并设置权限
echo -e "${YELLOW}📁 步骤 5/10: 创建必要目录并设置权限...${NC}"
...
# 7. 验证Docker环境
echo -e "${YELLOW}🔍 步骤 6/10: 验证Docker环境...${NC}"
...
# 8. 强制重新构建所有镜像
echo -e "${YELLOW}🔨 步骤 7/10: 强制重新构建Docker镜像...${NC}"
echo -e "${BLUE}这可能需要几分钟时间，请耐心等待...${NC}"

# 构建后端
echo -e "${YELLOW}🔨 构建后端镜像...${NC}"
docker-compose -f docker-compose.prod.yml build --no-cache --pull --force-rm backend

# 构建前端
echo -e "${YELLOW}🔨 构建前端镜像...${NC}"
docker-compose -f docker-compose.prod.yml build --no-cache --pull --force-rm frontend

# 9. 验证镜像构建成功
echo -e "${YELLOW}🔍 步骤 8/10: 验证镜像构建...${NC}"
...
if [ "$FRONTEND_IMAGE" -eq 0 ]; then
    ...
    docker-compose -f docker-compose.prod.yml build frontend 2>&1 | tail -30
    exit 1
fi
...
# 10. 启动服务
echo -e "${YELLOW}🚀 步骤 9/10: 启动服务...${NC}"
docker-compose -f docker-compose.prod.yml up -d

# 11. 等待并监控服务启动
echo -e "${YELLOW}⏳ 步骤 10/10: 等待服务启动并监控状态...${NC}"
for i in {1..12}; do
    echo "等待 ${i}0 秒... (${i}/12)"
    sleep 10
...
for url in "${FRONTEND_HEALTH_URLS[@]}"; do
    if curl -f -m 15 -s "$url" >/dev/null 2>&1; then
...
if [ "$FRONTEND_OK" = true ] && [ "$BACKEND_OK" = true ]; then
    echo -e "${GREEN}🎉 部署完全成功！所有服务都已正常运行${NC}"
...
echo "🧹 清理重来: docker system prune -a && ./final-fix-deploy.sh"
echo ""

# 16. 故障排除信息
if [ "$FRONTEND_OK" = false ] || [ "$BACKEND_OK" = false ]; then
    echo -e "${YELLOW}🔧 故障排除建议:${NC}"
    echo "1️⃣ 等待2-3分钟，服务可能还在启动"
    echo "2️⃣ 手动测试: curl http://localhost/health && curl http://localhost:8080/health"
    echo "3️⃣ 查看详细日志: docker-compose -f docker-compose.prod.yml logs -f"
    echo "4️⃣ 检查端口占用: netstat -tlnp | grep :80 && netstat -tlnp | grep :8080"
    echo "5️⃣ 进入容器检查: docker exec -it prompt-optimizer-backend-prod bash"
    echo "6️⃣ 重启Docker: systemctl restart docker"
    echo "7️⃣ 完全重置: docker system prune -a && ./final-fix-deploy.sh"
    echo ""
    echo -e "${YELLOW}📋 最近的服务日志:${NC}"
    echo -e "${BLUE}--- 后端日志 ---${NC}"
    docker-compose -f docker-compose.prod.yml logs --tail=10 backend 2>/dev/null || echo "无法获取后端日志"
    echo -e "${BLUE}--- 前端日志 ---${NC}"
    docker-compose -f docker-compose.prod.yml logs --tail=10 frontend 2>/dev/null || echo "无法获取前端日志"
else
    echo -e "${GREEN}🎯 恭喜！部署成功，所有问题已解决！${NC}"
    echo -e "${GREEN}🚀 您的LLM提示词优化平台已准备就绪！${NC}"
    echo ""
    echo -e "${PURPLE}📝 下一步:${NC}"
